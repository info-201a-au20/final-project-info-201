...
draw_map <- function(energy_per_state) {
  ...
  map_recp <- leaflet(data = energy_per_state) %>%
    ...
    addCircleMarkers(
      radius = ~ (Total_Renewable_Energy /
                    (Total_fossil_fuel + Total_Renewable_Energy)) * 80,
      lat = ~lat,
      lng = ~long,
      stroke = FALSE,
      fillOpacity = 0.6,
      popup = paste("net energy consumption = ",
                    energy_per_state$Total_fossil_fuel +
                      energy_per_state$Total_Renewable_Energy,
